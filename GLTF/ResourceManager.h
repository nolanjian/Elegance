...
namespace Elegance
{
	namespace GLTF
	{
		...
		{
		public:
			explicit ResourceManager(std::shared_ptr<fx::gltf::Document> gltfObject, const std::string& strPath);

			osg::ref_ptr<osg::Array> GetArrayByAccessorIndex(size_t index);
			osg::ref_ptr<osg::Texture> GetTexture(size_t index);
			osg::ref_ptr<osg::Geode> GetMesh(size_t index);
		...
		private:
			const std::string m_strGLTFPath;
