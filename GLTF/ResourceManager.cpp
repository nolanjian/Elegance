...
#define METALLIC_ROUGHNESS_TEXTURE_INDEX 1
#define NORMAL_TEXTURE_INDEX 2
#define OCCLUSION_TEXTURE_INDEX 3
#define EMISSIVE_TEXTURE_INDEX 4

#define BASE_COLOR_FACTOR "baseColorFactor"
#define USE_BASE_COLOR_FACTOR "useBaseColorFactor"

#define BASE_COLOR_TEXTURE "baseColorTexture"
#define BASE_COLOR_TEXTURE_COORD "baseColorTextureCoord"
#define USE_BASE_COLOR_TEXTURE "useBaseColorTexture"

#define NORMAL_TEXTURE "normalTexture"
#define NORMAL_TEXTURE_COORD "normalTextureCoord"
#define NORMAL_TEXTURE_SCALE "normalTextureScale"
#define USE_NORMAL_TEXTURE "useNormalTexture"

#define METALLIC_ROUGHNESS_TEXTURE "metallicRoughnessTexture"
#define METALLIC_ROUGHNESS_TEXTURE_COORD "metallicRoughnessTextureCoord"
#define USE_METALLIC_ROUGHNESS_TEXTURE "useMetallicRoughnessTexture"

#define ROUGHNESS_FACTOR "roughnessFactor"
#define METALLIC_FACTOR "metallicFactor"

#define OCCLUSION_TEXTURE "occlusionTexture"
#define OCCLUSION_TEXTURE_COORD "occlusionTextureCoord"
#define OCCLUSION_TEXTURE_STRENGTH "occlusionTextureStrength"
#define USE_OCCLUSION_TEXTURE "useOcclusionTexture"

#define EMISSIVE_FACTOR "emissiveFactor"
#define EMISSIVE_TEXTURE "emissiveTexture"
#define EMISSIVE_TEXTURE_COORD "emissiveTextureCoord"
#define USE_EMISSIVE_TEXTURE "useEmissiveTexture"

#define MATERIAL_DOUBLE_SIDED "materialDoubleSided"

#define ALPHA_CUTOFF "alphaCutoff"
#define ENABLE_ALPHA_CUTOFF "enableAlphaCutoff"

#define USE_VERTEX_COLOR "useVertexColor"

#define USE_TANGENT "useTangent"
#define USE_TEXCOORD0 "useTexcoord0"
#define USE_TEXCOORD1 "useTexcoord1"

namespace Elegance
{
	namespace GLTF
	{
		ResourceManager::ResourceManager(std::shared_ptr<fx::gltf::Document> gltfObject, const std::string& strPath)
			: m_gltfObject(gltfObject), m_strGLTFPath(strPath)
		{
			LoadArray();
			LoadTexture();
			LoadMesh();
		...
		osg::ref_ptr<osg::Array> ResourceManager::GetArrayByAccessorIndex(size_t index)
		{
		...
		osg::ref_ptr<osg::Texture> ResourceManager::GetTexture(size_t index)
		{
		...
		osg::ref_ptr<osg::Geode> ResourceManager::GetMesh(size_t index)
		{
		...
		void ResourceManager::LoadArray()
		{
		...
		osg::ref_ptr<osg::RefMatrix> ResourceManager::GetOSGMatrix(const fx::gltf::Accessor& accessor)
		{
		...
		osg::ref_ptr<osg::Array> ResourceManager::GetOSGArray(const fx::gltf::Accessor& accessor)
		{
		...
		inline osg::ref_ptr<osg::Array> ResourceManager::GetOSGVecArrayImp(const fx::gltf::Accessor& accessor)
		{
		...
		template<class ArrayType>
		inline osg::ref_ptr<osg::Array> ResourceManager::GetOSGArrayImp(const fx::gltf::Accessor& accessor)
		{
			osg::ref_ptr<ArrayType>	pRet = new ArrayType();
		...
		void ResourceManager::LoadSampler(osg::ref_ptr<osg::Texture2D> pTexture, const fx::gltf::Sampler& sampler)
		{
		...
		void ResourceManager::LoadImage(osg::ref_ptr<osg::Texture2D> pTexture, const fx::gltf::Image& image)
		{
			if (pTexture == nullptr)
			{
				return;
			...
			try
			{
				...
				auto fnLoadEmbeddedResource = [&]() -> bool
				{
					try
					{
						...
						pImage = Utils::LoadImageFromMemory(&imgData[0]);
				...
				auto fnLoadExteralResource = [&]() -> bool
				{
					try
					{
						...
						pImage = Elegance::Utils::LoadImageFromPath(filePath);
						return pImage != nullptr;
					}
					catch (const std::exception& e)
					{
				...
				auto fnLoadBufferViewResource = [&]() -> bool
				{
					try
					{
						if (image.bufferView >= 0)
						{
							...
							pImage = Elegance::Utils::LoadImageFromMemory(const_cast<unsigned char*>(pData));
							return pImage != nullptr;
						}
						return false;
					}
		...
		osg::ref_ptr<osg::Drawable> ResourceManager::LoadDrawable(const fx::gltf::Primitive& primitive)
		{
		...
		osg::ref_ptr<osg::PrimitiveSet> ResourceManager::LoadPrimitiveSet(const fx::gltf::Primitive& primitive)
		{
		...
		osg::ref_ptr<osg::DrawElements> ResourceManager::LoadDrawElements(const fx::gltf::Primitive& primitive)
		{
		...
		bool ResourceManager::LoadMaterial(const fx::gltf::Material& material, osg::ref_ptr<osg::Geometry> pGeometry)
		{
		...
		bool ResourceManager::LoadPBRTexture(const fx::gltf::Material::PBRMetallicRoughness& pbrMaterial, osg::ref_ptr<osg::Geometry> pGeometry)
		{
		...
		bool ResourceManager::LoadNormalTexture(const fx::gltf::Material::NormalTexture& normalTexture, osg::ref_ptr<osg::Geometry> pGeometry)
		{
		...
		bool ResourceManager::LoadOcclusionTexture(const fx::gltf::Material::OcclusionTexture& occlusionTexture, osg::ref_ptr<osg::Geometry> pGeometry)
		{
